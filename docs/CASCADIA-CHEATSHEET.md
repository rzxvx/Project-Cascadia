...
**Ключ клиента** тоже больше не протухает: `./cascadia build` каждый раз
дописывает в `/root/.ssh/authorized_keys` образа ключ машины, которая собирает
(`~/.ssh/id_ed25519.pub`, или `PUBKEY=...`). Раньше его клал только
`add-dropbear.sh` при `./cascadia rootfs`, и после нового `ssh-keygen` образ
встречал логин запросом пароля, которого нет (2026-09-19: в образе был
`SHA256:yicA...`, клиент предлагал `SHA256:j+r6...`; на NFS-корне лежал уже
новый ключ, поэтому там всё работало). «User 'root' has blank password,
rejected» в `dmesg` — это только отказ входа по паролю, к ключам отношения не
имеет.
...
./cascadia nfs on          # экспорт корня с хоста + /etc/nfsroot в initramfs
                           # мак: ~/cascadia-root, Linux: /srv/cascadia-root
./cascadia build && ./cascadia flash

# каждый раз после прошивки -- адрес 10.55.0.1 на хосте flash выставляет сам
...
| повторный `nfs on` | экспорт теперь ЖИВОЙ корень; `rsync --delete` снёс бы всё установленное. Оба скрипта (мак и Linux) отказываются перезаписывать непустой каталог без `FORCE_SYNC=1` |
| Linux-хост с docker | docker ставит политику FORWARD в DROP, и accept в своей nft-таблице пакет не спасает: его должны пропустить все базовые цепочки хука. `linux-share-internet.sh` вставляет свои правила в сам FORWARD, с комментарием `cascadia`, `off` удаляет ровно их |
| Linux-хост с NetworkManager | NM забирает новый ethernet-интерфейс гаджета, 45 с гоняет DHCP, сдаётся и снимает адрес — на Ubuntu посреди загрузки: NFS «server not responding», ssh висит, потом «No route to host». Пометка unmanaged гонку проигрывала. `host-link.sh` один раз создаёт профиль NM `cascadia-usb` (MAC гаджета, 10.55.0.1/24, без default route), дальше адрес ставит сам NM на каждой энумерации. Удалить: `sudo nmcli connection delete cascadia-usb` |
| ufw «running», хотя выключен | `ufw.service` висит в `active (exited)` и при выключенном ufw; включён он или нет — `ENABLED=yes` в `/etc/ufw/ufw.conf`, по нему скрипты и смотрят |
| iPad не пингует сам себя (10.55.0.2) | `lo` никто не поднимал, а свои адреса Linux достаёт через lo. Stage 1 теперь поднимает lo |
...
# ═══════════════════════════════════════════════════════════════
# ТАЧ РАБОТАЕТ (2026-09-25) — /dev/input/event0 с загрузки
# ═══════════════════════════════════════════════════════════════

Всё, что написано выше про тач («события не идут», Cmwp, SPI1 abort), устарело.
Детали — `docs/research/p105-z2-boot.md`, здесь только то, что нужно помнить.

**Как устроено.** stage2 запускает `/sbin/z2-boot -c /lib/firmware/mtcal.bin`:
он загружает чип так же, как iOS, и передаёт его драйверу `apple-z2` (`READY`),
...
- умолчания загрузчика N1: `0x1000305c <- 0x20`, `0x10003000 <- 3` (никогда не «0 по адресу 0»:
  это reset-вектор прошивки, после EXECUTE будет `4f81`);
- после EXECUTE: `ee`, `e2`×2, GET `d1 d3 d0 a1 d9`, затем **SET `bf = 9b 0b 0b 02`, `af = 00`** ×2 —
  их в iOS ставит userspace, они и запускают сканирование;
- **LDO idx 15 PMU** (`0x22` бит `0x02`, 5-вольтовая группа) — нет ни в одной функции ADT,
  iOS держит включённым; в DTS идёт вместе с аналоговой шиной (`touch_ana`, маска `0x06`);
- чтение кадра как в iOS, с повтором: первый кадр касания часто «не готов».

**Эталон из iOS, без патчей:** `build/mtlog` — собственная трассировка драйвера тача
(каждый шаг и каждая SPI-передача) через его user client; блокировка и разблокировка
iPad перезагружают чип в этот же лог. Регистры PMU живой iOS — `mtdump`,
свойство `AppleRegisterDump` класса `AppleARMPMUCharger`.

**Попутно:** `irq-apple-aic1.c` больше не выключает весь AIC от одного пустого
прерывания — раньше именно это роняло USB/сеть в момент старта сканирования.

**Отладка:** `echo 300 > /sys/module/apple_z2/parameters/debug` включает трассировку
кадров и касаний в dmesg; `z2-boot -p N -N` читает кадры сам, без ядра.
