...
| Linux-хост с NetworkManager | NM забирает новый ethernet-интерфейс гаджета, 45 с гоняет DHCP, сдаётся и снимает адрес. `host-link.sh` помечает интерфейс unmanaged (только на время жизни интерфейса) перед тем, как дать адрес |
